...
export default function ProductDetailMargin({statusReload, dateRange, productType}:ParamProps) { 
    ...
    chart: {
      ...
      events: {
        dataPointMouseEnter: function (event, chartContext, opts) { 
          ...
          createEventLog(cookies.get('name')!, '/otomax/product', 'margin|barchart', 'hover', JSON.stringify(value))
        }
      }
    },
    plotOptions: {
  ...
  const changeDateType = (newValue: string) => {  
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/product', 'margin|tab-date-type', 'change', newValue)
    }
  }; 
  ...
  const changeLevelType = (newValue: string) => { 
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/product', 'margin|tab-level-type', 'change', newValue)
    }
  };  
  ...
  async function fetchDataBigNumber() {
    ...
    await fetch('/api/otomax/product/big-number-margin?start_date='+paramStartDate+'&end_date='+paramEndDate+'&product_type='+paramProductType).then((response) => {
  ...
  async function fetchDataSeries() {
    ...
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
    const paramProductType = productType
		await fetch('/api/otomax/product/series-margin?start_date='+paramStartDate+'&end_date='+paramEndDate+'&product_type='+paramProductType+'&date_type='+typeDate.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
  ...
  async function fetchProductDetail() {
    ...
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')     
    const paramProductType = productType
		await fetch('/api/otomax/product/detail-margin?start_date='+paramStartDate+'&end_date='+paramEndDate+'&product_type='+paramProductType+'&date_type='+typeDate.current+'&level_type='+typeLevel.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
        ...
        <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
          <div className="w-full">
            ...
            <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
              Nilai margin dibandingan dengan product
            </p>
          </div>  
              ...
              { 
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.margin_per_product} thousandSeparator="." decimalSeparator="," decimalScale={0} prefix="Rp. "/>
                      </h4> 
                    </div> 
                  </div>
                )
