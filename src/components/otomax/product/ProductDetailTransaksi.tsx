...
export interface BigNumberModel {
  transaction_all: number; 
  product_active: number;
  transaction_per_product: number;
  transaction_h2h: number;
  transaction_h2h_percent: number;
  transaction_retail: number;
  transaction_retail_percent: number;
  transaction_website: number;
  transaction_website_percent: number; 
}
...
export default function ProductDetailTransaksi({statusReload, dateRange, productType}:ParamProps) { 
    ...
    chart: {
      ...
      events: {
        dataPointMouseEnter: function (event, chartContext, opts) { 
          ...
          createEventLog(cookies.get('name')!, '/otomax/product', 'transaksi|barchart', 'hover', JSON.stringify(value))
        }
      }
    },
    plotOptions: {
  ...
  const changeDateType = (newValue: string) => {  
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/product', 'transaksi|tab-date-type', 'change', newValue)
    }
  }; 
  ...
  const changeLevelType = (newValue: string) => { 
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/product', 'transaksi|tab-level-type', 'change', newValue)
    }
  };  
  ...
  async function fetchDataBigNumber() {
    ...
    await fetch('/api/otomax/product/big-number-transaction?start_date='+paramStartDate+'&end_date='+paramEndDate+'&product_type='+paramProductType).then((response) => {
  ...
  async function fetchDataSeries() {
    ...
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
    const paramProductType = productType
		await fetch('/api/otomax/product/series-transaction?start_date='+paramStartDate+'&end_date='+paramEndDate+'&product_type='+paramProductType+'&date_type='+typeDate.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
  ...
  async function fetchProductDetail() {
    ...
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')     
    const paramProductType = productType
		await fetch('/api/otomax/product/detail-transaction?start_date='+paramStartDate+'&end_date='+paramEndDate+'&product_type='+paramProductType+'&date_type='+typeDate.current+'&level_type='+typeLevel.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
    ...
    <div className="mt-5">
        ...
        <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
          <div className="w-full">
            ...
            <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
              Angka transaksi dibandingan dengan product
            </p>
          </div>  
        ...
        <div className="grid grid-cols-12 gap-4 md:gap-4"> 
                  ...
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.transaction_all} thousandSeparator="." decimalSeparator=","/>
                      </h4> 
                    </div> 
                  </div>
                )
          ...
          <div className="col-span-12 xl:col-span-4 space-y-3"> 
            <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-4">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-md text-gray-500 dark:text-gray-400">
                  Rata-rata Transaksi per Product
                </span> 
              </div> 
              { 
                loadingDataBigNumber || !DataBigNumberData ?
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.transaction_per_product} thousandSeparator="." decimalSeparator="," decimalScale={2}/>
                      </h4> 
                    </div> 
                  </div>
                )
          ...
          <div className="col-span-12 xl:col-span-4 space-y-3"> 
            <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-4">
              <div className="flex items-center space-x-3">
                <div className="flex items-center justify-center w-10 h-10 bg-yellow-500 rounded-xl dark:bg-green-800">
                  <LineIconCheckCircle /> 
                </div> 
                <span className="text-md text-gray-500 dark:text-gray-400">
                  Transaksi H2H
              ...
              { 
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.transaction_h2h} thousandSeparator="." decimalSeparator=","/>
                      </h4> 
                    </div>
                    {
                      DataBigNumberData?.transaction_h2h_percent > 0 ? 
                      ( 
                        <div className="flex">
                          <Badge color="success"> 
                            <NumericFormat displayType="text" value={DataBigNumberData?.transaction_h2h_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> %  
                          </Badge> 
                          <div className="leading-none ml-2">
                            dari <br/>transaksi <br/>total
                          </div> 
                        </div>
                      ) : 
                      (
              ...
              { 
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.transaction_retail} thousandSeparator="." decimalSeparator=","/>
                      </h4> 
                    </div>
                    {
                      DataBigNumberData?.transaction_retail_percent > 0 ? 
                      ( 
                        <div className="flex">
                          <Badge color="success"> 
                            <NumericFormat displayType="text" value={DataBigNumberData?.transaction_retail_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> %  
                          </Badge> 
                          <div className="leading-none ml-2">
                            dari <br/>transaksi <br/>total
                          </div> 
              ...
              { 
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.transaction_website} thousandSeparator="." decimalSeparator=","/>
                      </h4> 
                    </div>
                    {
                      DataBigNumberData?.transaction_website_percent > 0 ? 
                      ( 
                        <div className="flex">
                          <Badge color="success"> 
                            <NumericFormat displayType="text" value={DataBigNumberData?.transaction_website_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> %  
                          </Badge> 
                          <div className="leading-none ml-2">
                            dari <br/>transaksi <br/>total
                          </div> 
