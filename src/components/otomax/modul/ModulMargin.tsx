...
export interface BigNumberModel {
  margin_all: number;
  margin_stok_luar: number;
  margin_stok_luar_percent: number;
  margin_stok_sendiri: number;
  margin_stok_sendiri_percent: number; 
}
...
export default function ModulMargin({statusReload, dateRange}:StatusProps) {   
    ...
    chart: {
      ...
      events: {
        dataPointMouseEnter: function (event, chartContext, opts) { 
          ...
          createEventLog(cookies.get('name')!, '/otomax/modul', 'margin|barchart', 'hover', JSON.stringify(value))
        }
      }
    },
    plotOptions: {
  ...
  const changeModuleType = (newValue: string) => {  
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/modul', 'margin|tab-module-type', 'change', newValue)
    }
  };
  ...
  async function fetchModuleBigNumber() {
    ...
    await fetch('/api/otomax/module/big-number-margin?start_date='+paramStartDate+'&end_date='+paramEndDate).then((response) => {
  ...
  async function fetchModuleSeries() {
    ...
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
    await fetch('/api/otomax/module/series-margin?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current).then((response) => {
      if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
      }
      return response;
  ...
  async function fetchModuleDetail() {
    ...
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
    await fetch('/api/otomax/module/detail-margin?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current+'&module_type='+typeModule.current).then((response) => {
      if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
      }
      return response;
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:justify-between">
        <div className="w-full">
          ...
          <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
            Margin per modul, stok dalam, stok luar
          </p>
        </div>   
      </div>
                ...
                <div className="flow lg:flex items-end justify-between">  
                  <div>   
                    <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                      <NumericFormat displayType="text" value={dataBigNumber?.margin_stok_sendiri} thousandSeparator="." decimalSeparator="," prefix={'Rp. '}/>
                    </h4> 
                  </div> 
                  {
                    dataBigNumber?.margin_stok_sendiri_percent > 0 ? 
                    ( 
                      <div className="flex">
                        <Badge color="success"> 
                          <NumericFormat displayType="text" value={dataBigNumber?.margin_stok_sendiri_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> % 
                        </Badge> 
                        <div className="leading-none ml-2">
                          dari <br/>total margin
                        </div>
                      </div> 
                    ) : 
                    (
            ...
            </div>   
            {
              ...
              ( 
                <div className="flow lg:flex items-end justify-between">  
                  <div>   
                    <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                      <NumericFormat displayType="text" value={dataBigNumber?.margin_stok_luar} thousandSeparator="." decimalSeparator="," prefix={'Rp. '}/>
                    </h4> 
                  </div> 
                  {
                    dataBigNumber?.margin_stok_luar_percent> 0 ? 
                    ( 
                      <div className="flex">
                        <Badge color="success"> 
                          <NumericFormat displayType="text" value={dataBigNumber?.margin_stok_luar_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> % 
                        </Badge>
                        <div className="leading-none ml-2">
                          dari <br/>total margin
                        </div>
            ...
            </div>  
            {
              ...
              ( 
                <div className="flow lg:flex items-end justify-between">  
                  <div>   
                    <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                      <NumericFormat displayType="text" value={dataBigNumber.margin_all} thousandSeparator="." decimalSeparator="," prefix={'Rp. '}/>
                    </h4> 
                  </div>  
                </div> 
              )
