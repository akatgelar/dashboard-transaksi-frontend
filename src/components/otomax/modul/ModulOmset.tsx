...
export interface BigNumberModel {
  omset_all: number;
  omset_stok_luar: number;
  omset_stok_luar_percent: number;
  omset_stok_sendiri: number;
  omset_stok_sendiri_percent: number; 
}
...
export default function ModulOmset({statusReload, dateRange}:StatusProps) {   
    ...
    chart: {
      ...
      events: {
        dataPointMouseEnter: function (event, chartContext, opts) { 
          ...
          createEventLog(cookies.get('name')!, '/otomax/modul', 'omset|barchart', 'hover', JSON.stringify(value))
        }
      }
    },
    plotOptions: {
  ...
  const changeModuleType = (newValue: string) => {  
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/modul', 'omset|tab-module-type', 'change', newValue)
    }
  };
  ...
  async function fetchModuleBigNumber() {
    ...
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
		await fetch('/api/otomax/module/big-number-omset?start_date='+paramStartDate+'&end_date='+paramEndDate).then((response) => {
  ...
  async function fetchModuleSeries() {
    ...
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
		await fetch('/api/otomax/module/series-omset?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current).then((response) => {
      if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}
			return response;
  ...
  async function fetchModuleDetail() {
    ...
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
		await fetch('/api/otomax/module/detail-omset?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current+'&module_type='+typeModule.current).then((response) => {
      if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}
			return response;
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:justify-between">
        <div className="w-full">
          ...
          <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
            Omset per modul, stok dalam, stok luar
          </p>
        </div>   
      </div>
                ...
                <div className="flow lg:flex items-end justify-between">  
                  <div>   
                    <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                      <NumericFormat displayType="text" value={dataBigNumber?.omset_stok_sendiri} thousandSeparator="." decimalSeparator="," prefix={'Rp. '}/>
                    </h4> 
                  </div> 
                  {
                    dataBigNumber?.omset_stok_sendiri_percent > 0 ? 
                    ( 
                      <div className="flex">
                        <Badge color="success"> 
                          <NumericFormat displayType="text" value={dataBigNumber?.omset_stok_sendiri_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> % 
                        </Badge> 
            ...
            </div>   
            {
              ...
              ( 
                <div className="flow lg:flex items-end justify-between">  
                  <div>   
                    <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                      <NumericFormat displayType="text" value={dataBigNumber?.omset_stok_luar} thousandSeparator="." decimalSeparator="," prefix={'Rp. '}/>
                    </h4> 
                  </div> 
                  {
                    dataBigNumber?.omset_stok_luar_percent> 0 ? 
                    ( 
                      <div className="flex">
                        <Badge color="success"> 
                          <NumericFormat displayType="text" value={dataBigNumber?.omset_stok_luar_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> % 
                        </Badge>
                        <div className="leading-none ml-2">
                          dari <br/>total omset
                        </div>
            ...
            </div>  
            {
              ...
              ( 
                <div className="flow lg:flex items-end justify-between">  
                  <div>   
                    <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                      <NumericFormat displayType="text" value={dataBigNumber.omset_all} thousandSeparator="." decimalSeparator="," prefix={'Rp. '}/>
                    </h4> 
                  </div>  
                </div> 
              )
