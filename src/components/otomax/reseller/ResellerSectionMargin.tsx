...
export interface BigNumberModel {
  ...
  margin_per_reseller: number;
  margin_h2h: number;
  margin_h2h_percent: number;
  margin_retail: number;
  margin_retail_percent: number;
  margin_website: number;
  margin_website_percent: number; 
}
...
export default function ResellerSectionMargin({statusReload, dateRange}:ParamProps) { 
    ...
    chart: {
      ...
      events: {
        dataPointMouseEnter: function (event, chartContext, opts) { 
          ...
          createEventLog(cookies.get('name')!, '/otomax/reseller', 'margin|barchart', 'hover', JSON.stringify(value))
        }
      }
    },
    plotOptions: {
  ...
  const changeDateType = (newValue: string) => {  
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/reseller', 'margin|tab-date-type', 'change', newValue)
    }
  }; 
  ...
  const changeLevelType = (newValue: string) => { 
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/reseller', 'margin|tab-level-type', 'change', newValue)
    }
  };  
  ...
  async function fetchDataBigNumber() {
    ...
    await fetch('/api/otomax/reseller/big-number-margin?start_date='+paramStartDate+'&end_date='+paramEndDate).then((response) => {
  ...
  async function fetchDataSeries() {
    ...
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
		await fetch('/api/otomax/reseller/series-margin?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
  ...
  async function fetchResellerDetail() {
    ...
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')     
		await fetch('/api/otomax/reseller/detail-margin?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current+'&level_type='+typeLevel.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
    ...
    <div className="rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
        ...
        <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
          <div className="w-full">
            ...
            <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
              Nilai margin dibandingan dengan reseller
            </p>
          </div>  
        ...
        <div className="grid grid-cols-12 gap-4 md:gap-4"> 
                  ...
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.margin_all} thousandSeparator="." decimalSeparator="," prefix="Rp. "/>
                      </h4> 
                    </div> 
                  </div>
                )
              ...
              { 
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.margin_per_reseller} thousandSeparator="." decimalSeparator="," decimalScale={0} prefix="Rp. "/>
                      </h4> 
                    </div> 
                  </div>
                )
          ...
          <div className="col-span-12 xl:col-span-6 space-y-3"> 
            <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-4">
              <div className="flex items-center space-x-3">
                <div className="flex items-center justify-center w-10 h-10 bg-blue-500 rounded-xl dark:bg-green-800">
                  <LineIconCheckCircle /> 
                </div> 
                <span className="text-md text-gray-500 dark:text-gray-400">
                  Margin H2H
              ...
              { 
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.margin_h2h} thousandSeparator="." decimalSeparator="," prefix="Rp. "/>
                      </h4> 
                    </div>
                    {
                      DataBigNumberData?.margin_h2h_percent > 0 ? 
                      ( 
                        <div className="flex">
                          <Badge color="success"> 
                            <NumericFormat displayType="text" value={DataBigNumberData?.margin_h2h_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> % 
                          </Badge>
                          <div className="leading-none ml-2">
                            dari <br/>margin <br/>total
                          </div>
                        </div>
                      ) : 
                      (
              ...
              { 
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.margin_website} thousandSeparator="." decimalSeparator="," prefix="Rp. "/>
                      </h4> 
                    </div>
                    {
                      DataBigNumberData?.margin_website_percent > 0 ? 
                      ( 
                        <div className="flex">
                          <Badge color="success"> 
                            <NumericFormat displayType="text" value={DataBigNumberData?.margin_website_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> % 
                          </Badge> 
                          <div className="leading-none ml-2">
                            dari <br/>margin <br/>total
                          </div>
              ...
              { 
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.margin_retail} thousandSeparator="." decimalSeparator="," prefix="Rp. "/>
                      </h4> 
                    </div>
                    {
                      DataBigNumberData?.margin_retail_percent > 0 ? 
                      ( 
                        <div className="flex">
                          <Badge color="success"> 
                            <NumericFormat displayType="text" value={DataBigNumberData?.margin_retail_percent} thousandSeparator="." decimalSeparator="," decimalScale={2} allowNegative/> % 
                          </Badge> 
                          <div className="leading-none ml-2">
                            dari <br/>margin <br/>total
                          </div>
