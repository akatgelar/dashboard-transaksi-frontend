...
export interface BigNumberModel {
  ...
  transaction_per_reseller: number;
...
export default function ResellerSectionTransaksi({statusReload, dateRange}:ParamProps) { 
    ...
    chart: {
      ...
      events: {
        dataPointMouseEnter: function (event, chartContext, opts) { 
          ...
          createEventLog(cookies.get('name')!, '/otomax/reseller', 'transaksi|barchart', 'hover', JSON.stringify(value))
        }
      }
    },
    plotOptions: {
  ...
  const changeDateType = (newValue: string) => {  
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/reseller', 'transaksi|tab-date-type', 'change', newValue)
    }
  }; 
  ...
  const changeLevelType = (newValue: string) => { 
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/reseller', 'transaksi|tab-level-type', 'change', newValue)
    }
  };  
  ...
  async function fetchDataBigNumber() {
    ...
    await fetch('/api/otomax/reseller/big-number-transaction?start_date='+paramStartDate+'&end_date='+paramEndDate).then((response) => {
  ...
  async function fetchDataSeries() {
    ...
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
		await fetch('/api/otomax/reseller/series-transaction?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
  ...
  async function fetchResellerDetail() {
    setLoadingDataDetail(true)
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')     
		await fetch('/api/otomax/reseller/detail-transaction?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current+'&level_type='+typeLevel.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
        ...
        if (result["data"] != null) {
          ...
          tempColumn.push({field: 'name',headerName: 'Reseller', minWidth: 200, filter: true, pinned: 'left'});
        ...
        <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
          <div className="w-full">
            ...
            <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
              Angka transaksi dibandingan dengan reseller
            </p>
          </div>  
          ...
          <div className="col-span-12 xl:col-span-6 space-y-3"> 
            <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-4">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-md text-gray-500 dark:text-gray-400">
                  Rata-rata Transaksi per Reseller
                </span> 
              </div> 
              { 
                loadingDataBigNumber || !DataBigNumberData ?
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.transaction_per_reseller} thousandSeparator="." decimalSeparator="," decimalScale={2}/>
                      </h4> 
                    </div> 
                  </div>
                )
