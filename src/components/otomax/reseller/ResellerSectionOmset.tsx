...
export default function ResellerSectionOmset({statusReload, dateRange}:ParamProps) { 
    ...
    chart: {
      ...
      events: {
        dataPointMouseEnter: function (event, chartContext, opts) { 
          ...
          createEventLog(cookies.get('name')!, '/otomax/reseller', 'omset|barchart', 'hover', JSON.stringify(value))
        }
      }
    },
    plotOptions: {
  ...
  const changeDateType = (newValue: string) => {  
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/reseller', 'omset|tab-date-type', 'change', newValue)
    }
  }; 
  ...
  const changeLevelType = (newValue: string) => { 
    ...
    if (newValue) {  
      ...
      createEventLog(cookies.get('name')!, '/otomax/reseller', 'omset|tab-level-type', 'change', newValue)
    }
  };  
  ...
  async function fetchDataBigNumber() {
    ...
    await fetch('/api/otomax/reseller/big-number-omset?start_date='+paramStartDate+'&end_date='+paramEndDate).then((response) => {
  ...
  async function fetchDataSeries() {
    ...
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')  
		await fetch('/api/otomax/reseller/series-omset?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
  ...
  async function fetchResellerDetail() {
    ...
    const paramStartDate = moment(dateRange[0]).format('YYYY-MM-DD')  
    const paramEndDate = moment(dateRange[1]).format('YYYY-MM-DD')     
		await fetch('/api/otomax/reseller/detail-omset?start_date='+paramStartDate+'&end_date='+paramEndDate+'&date_type='+typeDate.current+'&level_type='+typeLevel.current).then((response) => {
			if (response.status >= 500 && response.status < 600) {
        toast.error(response.statusText)
			}  
			return response;
        ...
        <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
          <div className="w-full">
            ...
            <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
              Nilai omset dibandingan dengan reseller
            </p>
          </div>  
              ...
              { 
                ...
                (
                  <div className="flow lg:flex items-end justify-between">  
                    <div>   
                      <h4 className="mt-2 font-bold text-gray-800 text-title-md dark:text-white/90">
                        <NumericFormat displayType="text" value={DataBigNumberData.omset_per_reseller} thousandSeparator="." decimalSeparator="," decimalScale={0} prefix="Rp. "/>
                      </h4> 
                    </div> 
                  </div>
                )
